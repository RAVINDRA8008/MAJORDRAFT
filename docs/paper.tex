...
\begin{itemize}[leftmargin=*, nosep, label=\textcolor{hlgreen}{$\blacktriangleright$}]
    ...
    \item 82.55\% accuracy and $\sim$0.83 macro F1 on 4-class emotion recognition (DEAP + IEMOCAP); 68.41\% under 32-fold LOSO
\end{itemize}
\end{keybox}
...
To assess cross-subject generalization, we conduct a 32-fold leave-one-subject-out (LOSO) evaluation on DEAP. In each fold, one subject is held out as the test set, the next subject serves as the validation set, and the remaining 30 subjects form the training set. IEMOCAP speech data is split 80/20 across all folds (constant). Each fold trains from scratch with reduced hyperparameters (40 epochs, patience~10) using fresh encoder weights initialized from pretrained checkpoints.

\vspace{2pt}
\begin{center}
\renewcommand{\arraystretch}{1.30}
\small
\begin{tabular}{@{\hspace{6pt}}l@{\hspace{16pt}}r@{\hspace{6pt}}}
\toprule
\rowcolor{headerblue!10}
\textcolor{headerblue}{\textbf{Metric}} & \textcolor{headerblue}{\textbf{Value}} \\
\midrule
Mean accuracy & 68.41\% $\pm$ 8.37\% \\
\rowcolor{tablegray} Median accuracy & 70.29\% \\
Range & [48.58\%, 86.46\%] \\
\rowcolor{tablegray} Mean F1 (macro) & 0.575 $\pm$ 0.059 \\
Best fold (subject 12) & 86.46\% \\
\rowcolor{tablegray} Worst fold (subject 5) & 48.58\% \\
\bottomrule
\end{tabular}
\end{center}
\vspace{-2pt}

The $\sim$14\,pp drop from subject-dependent (82.55\%) to subject-independent (68.41\%) evaluation is consistent with the well-documented individual variability in EEG signals~\cite{li2018eeg}. Per-class pooled results reveal that sad (F1~=~0.73) and angry (F1~=~0.85) generalize well across subjects, while happy---the extreme minority class (1,140 samples, 1.5\% of DEAP)---collapses to F1~=~0.12 under LOSO, confirming that the subject-dependent evaluation masked its fragility. The high fold variance (std~=~8.37\%) reflects known inter-subject EEG differences in the DEAP dataset.
...
\begin{itemize}[leftmargin=*, label=\textcolor{hlred}{$\blacktriangleright$}, itemsep=3pt]
    ...
    \item \textbf{Subject-dependent vs.\ subject-independent gap}: Our 32-fold LOSO evaluation yields 68.41\% $\pm$ 8.37\% mean accuracy---a $\sim$14\,pp drop from the subject-dependent 82.55\%. This confirms that the randomized split overestimates cross-subject performance. Furthermore, encoder pretraining (DANN, contrastive) used all 32 subjects, introducing partial data leakage into per-fold LOSO. A fully leak-free protocol would re-pretrain encoders per fold.
    ...
    \item \textbf{Residual imbalance}: Happy has only 1,140 EEG samples (high-valence, high-arousal quadrant) vs.\ 41,940 for sad, limiting minority-class encoder quality. Under LOSO, happy collapses to F1~=~0.12 while subject-dependent evaluation masked this fragility.
...
We presented AMERS, an adaptive multimodal emotion recognition system that fuses EEG and speech through Cross-Modal Mutual Attention with Emotion-Aware Gating. Across five major versions and nine CMMA sub-versions, we achieved \textbf{82.55\%} accuracy on four-class recognition, exceeding targets of 75\% accuracy and 0.72 macro F1. A 32-fold LOSO evaluation confirms subject-independent generalization at \textbf{68.41\% $\pm$ 8.37\%}, with the $\sim$14\,pp gap reflecting known EEG inter-subject variability.
...
\begin{enumerate}[leftmargin=*, label=\textcolor{headerblue}{\arabic*.}, itemsep=2pt]
    ...
    \item \textbf{Leak-free LOSO} --- Re-pretrain encoders per fold to eliminate the pretraining data leak identified in our current LOSO protocol.

...
\begin{itemize}[leftmargin=*, nosep, label=\textcolor{hlgreen}{$\blacktriangleright$}]
    ...
    \item 82.55\% accuracy and $\sim$0.83 macro F1 on 4-class emotion recognition (DEAP + IEMOCAP)
\end{itemize}
\end{keybox}
...
\begin{itemize}[leftmargin=*, label=\textcolor{hlred}{$\blacktriangleright$}, itemsep=3pt]
    ...
    \item \textbf{Subject-dependent evaluation}: Our evaluation uses randomized train/validation splits that may include same-subject EEG data across partitions. Leave-one-subject-out (LOSO) cross-validation is the gold standard for EEG generalization claims~\cite{li2018eeg}. Our reported accuracy \emph{likely overestimates} cross-subject performance; LOSO evaluation would almost certainly yield lower accuracy (we estimate 70--76\% based on typical DEAP subject-dependent vs.\ subject-independent gaps in the literature) but would provide more credible generalization evidence. We leave LOSO evaluation for future work.

...
\begin{enumerate}[leftmargin=*, label=\textcolor{headerblue}{\arabic*.}, itemsep=2pt]
    ...
    \item \textbf{Leak-free LOSO} --- Re-pretrain encoders per fold to eliminate the pretraining data leak identified in our current LOSO protocol. Our improved LOSO v2 (89.86\%) demonstrates that architectural improvements compound with proper evaluation methodology. We have implemented a strict LOSO protocol (\texttt{strict\_loso\_v2.py}) that retrains contrastive and DANN encoders per fold using only the 30 training subjects; results pending.

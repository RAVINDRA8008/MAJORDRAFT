...
\addplot[fill=eegcolor!60, draw=eegcolor!85, line width=0.5pt] coordinates
    {(Angry,0.57) (Happy,0.50) (Sad,0.43) (Neutral,0.44)};
\addplot[fill=speechcolor!60, draw=speechcolor!85, line width=0.5pt] coordinates
    {(Angry,0.43) (Happy,0.50) (Sad,0.57) (Neutral,0.56)};
\legend{EEG Weight, Speech Weight}
% Equal-weighting reference line
\draw[dashed, darkgray!40, line width=0.5pt] (axis cs:Angry,0.5) -- (axis cs:Neutral,0.5)
    node[pos=1, anchor=west, font=\tiny, color=darkgray!60] {~0.50};
\end{axis}
\end{tikzpicture}
...
\begin{axis}[
    ...
    xlabel style={font=\footnotesize},
    ylabel={Accuracy (\%)},
    ylabel style={font=\footnotesize, color=headerblue, at={(0.06,0.5)}},
    xmin=0, xmax=80,
    ymin=20, ymax=100,
    xtick={0,10,20,30,40,50,60,70,80},
    xticklabel style={font=\footnotesize},
    yticklabel style={font=\footnotesize, color=headerblue},
    axis y line*=left,
    axis x line*=bottom,
    grid=major,
...
\addplot[headerblue!10, forget plot, draw=none, fill opacity=0.45]
    coordinates {
...
% Training accuracy
\addplot[line width=1.1pt, color=headerblue, mark=*, mark size=1.2pt,
...
% Validation accuracy
\addplot[line width=1.1pt, color=hlgreen, mark=triangle*, mark size=1.5pt,
...
% TF ratio (plotted here for unified legend)
\addplot[line width=1pt, color=eagcolor, densely dashed, mark=none, forget plot]
    coordinates {(0,0) (0,0)}; % dummy for spacing

% Best epoch vertical marker
\draw[line width=0.6pt, dashed, goldaccent] (axis cs:43,20) -- (axis cs:43,85);
\node[fill=white, draw=goldaccent, rounded corners=2pt, inner sep=3pt,
      font=\footnotesize\bfseries, text=goldaccent]
    at (axis cs:62,28) {Best: 82.55\%};
\node[font=\scriptsize, text=goldaccent] at (axis cs:62,23) {(epoch 43)};
\draw[-{Stealth[length=4pt]}, goldaccent, line width=0.5pt]
    (axis cs:56,28) -- (axis cs:44,28);

% Overfitting gap annotation
\draw[<->, headerblue!55, line width=0.5pt] (axis cs:72,81.0) -- (axis cs:72,95.5);
\node[font=\scriptsize, color=headerblue!65, anchor=west] at (axis cs:73,88.2) {gap};
...
\begin{axis}[
    ...
    ymin=0, ymax=1.15,
    ylabel={TF Ratio ($\rho$)},
    ylabel style={font=\footnotesize, color=eagcolor, at={(0.95,0.5)}},
    yticklabel style={font=\footnotesize, color=eagcolor},
    ytick={0, 0.25, 0.5, 0.75, 1.0},
    axis y line*=right,
    axis x line=none,
    legend style={
        at={(0.02,0.67)}, anchor=north west,
        font=\footnotesize,
        draw=tableborder,
        fill=white,
        fill opacity=0.93,
...
% TF shading
\addplot[eagcolor!12, forget plot, draw=none, fill opacity=0.4]
...
\end{axis}
\end{tikzpicture}
\caption{Training dynamics for v5.3. The shaded gap between training ($\sim$95\%) and validation ($\sim$82.5\%) reflects limited speech data (4,424 samples). Teacher forcing (orange shading) decays to zero by epoch 25, after which the model uses its own probe for gating.}
...
\begin{axis}[
    ...
    ylabel style={font=\footnotesize, at={(0.07,0.5)}},
    ...
    enlarge x limits=0.20,
    clip=false,
    % No grouped bar offset — single series, properly centered
    visualization depends on={value \thisrowno{0} \as \myval},
]
% Single \addplot with all bars → properly centered on x-labels
\addplot[fill=headerblue!55, draw=headerblue!85, line width=0.5pt] coordinates {
    (v1,25) (v2,55.92) (v3,65.97) (v4,81.94) (v5.3,82.55)
};

% Highlight best bar (v5.3) with green overlay
\fill[hlgreen!30, draw=hlgreen!80, line width=0.8pt, rounded corners=0.5pt]
    (axis cs:v5.3,0) +(-10pt,0) rectangle +(10pt,82.55pt);
% Redraw the value label for v5.3 in green
\node[font=\scriptsize\bfseries, color=hlgreen!80!black, anchor=south]
    at (axis cs:v5.3,82.55) {82.55};

% Star on best
\node[font=\footnotesize, color=hlgreen, anchor=south] at (axis cs:v5.3,86)
    {$\bigstar$ \scriptsize\textbf{Best}};

% Annotation arrow for biggest jump (v3→v4)
\draw[-{Stealth[length=5pt]}, line width=0.8pt, hlgreen]
    (axis cs:v3,72) -- (axis cs:v4,72);
\node[font=\scriptsize\bfseries, color=hlgreen, fill=white, inner sep=1.5pt]
    at ($(axis cs:v3,72)!0.5!(axis cs:v4,72) + (0,5pt)$) {$+$15.97\,pp};
\node[font=\scriptsize, color=hlgreen!75, anchor=north]
...
\begin{tikzpicture}[scale=0.82]
    ...
    \foreach \i in {0,...,3} {
        \foreach \j in {0,...,3} {
            ...
            \ifnum\i=\j
                \pgfmathsetmacro{\intensity}{max(18, \val * 0.92)}
                \fill[hlgreen!\intensity!white] (\j, -\i) rectangle (\j+1, -\i-1);
            \else
                \pgfmathsetmacro{\intensity}{min(60, \val * 3.2)}
                \fill[hlred!\intensity!white] (\j, -\i) rectangle (\j+1, -\i-1);
            \fi
            % Cell value — consistent \footnotesize for all cells
            \pgfmathsetmacro{\valint}{int(\val)}
            \ifnum\i=\j
                \node[font=\footnotesize\bfseries, color=white!15!black]
                    at (\j+0.5, -\i-0.5) {\valint\%};
            \else
                \pgfmathtruncatemacro{\v}{\val}
                \ifnum\v>10
                    \node[font=\footnotesize\bfseries, color=hlred!75!black]
                        at (\j+0.5, -\i-0.5) {\valint\%};
                \else
                    \node[font=\footnotesize, color=darkgray!75]
                        at (\j+0.5, -\i-0.5) {\valint\%};
                \fi
            \fi
        }
    ...
    \foreach \i in {1,...,3} {
        \draw[white, line width=1.4pt] (\i, 0) -- (\i, -4);
        \draw[white, line width=1.4pt] (0, -\i) -- (4, -\i);
    }
    % Outer border
    \draw[headerblue, line width=1.2pt, rounded corners=1pt] (0,0) rectangle (4,-4);
    ...
    \node[font=\footnotesize\bfseries, color=headerblue, rotate=90, anchor=south]
        at (-1.2, -2) {True};
